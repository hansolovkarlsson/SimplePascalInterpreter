...
namespace SPI
{
    ...
    class Parser
    {
        ...
        public void Eat(Token.TYPE token_type)
        {
            ...
            */
            if(current_token!.type==token_type)
                ...
                Debug.Error($"Surprise: 'eat' wrong token type {current_token.Str()}!={token_type.ToString()}");
        }
        ...
        public List<AST> Declarations()
        {
            ...
            while(current_token.type == Token.TYPE.PROCEDURE) {
                Eat(Token.TYPE.PROCEDURE);
                string proc_name = current_token.value!;
                Eat(Token.TYPE.ID);
                Eat(Token.TYPE.SEMI);
                NodeBlock block_node = Block();
                NodeProcedureDecl proc_decl = new NodeProcedureDecl(proc_name, block_node);
