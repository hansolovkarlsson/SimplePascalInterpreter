...
namespace SPI
{
    ...
    abstract class NodeVisitor
    {
        ...
        {
            Debug.Trace(Debug.TRACE.VISITOR, $"Visit({node.token.Str()}");
            ...
            Debug.Trace(Debug.TRACE.VISITOR, $"class_type {class_type.ToString()}");

            Type node_type = node.GetType();
            string? method_name = node_type.ToString();
            method_name = method_name.Replace("SPI.", "Visit_");

            Debug.Trace(Debug.TRACE.VISITOR, $"method_name={method_name!}");
        ...
        public abstract dynamic Visit_NodeProcedureCall(NodeProcedureCall node);
        public abstract dynamic Visit_NodeWriteStatement(NodeWriteStatement node);
        ...
        public abstract dynamic Visit_NodeStr(NodeStr node);
